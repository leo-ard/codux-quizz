...
import './assets/Pixel.otf'

import {
    Personnalite,
    PersonnaliteProps,
    PersonnaliteStr,
} from './components/personnalite/personnalite';
import { totalmem } from 'os';

function check_tu_vous(answer_nb : number[]){
...
function get_question(
    ...
    let tu_str = check_tu_vous(answer_nb);
    ...
    if (tu_str == 'vous' && nb == 4) {
        ...
        return {
            question:
                `Si un jour ${tu('tu mourrais', 'vous mourriez')}, ` +
                answers[0] +
                `, en quoi ${tu('voudrais-tu', 'voudriez-vous')} le plus ${tu(
                    'te',
                    'vous'
                )} réincarner?`,
            points: [
        ...
        return {
            question: ` Intéressant... Et comment ${tu(
                'voudrais-tu',
                'voudriez-vous'
            )} que les gens se souviennent de ${tu('toi', 'vous')}?`,
            points: [
                { reveur: 2 },
                { robot: 2 },
                { leader: 2 },
        ...
        return {
            question:
                ' Entre toutes les activités ci-dessous, ' +
                answers[0] +
                `, laquelle ${tu('te', 'vous')} semble la plus attirante?`,
            points: [
                { robot: 2, reveur: 1 },
                { survivant: 2, robot: 1 },
                { hermite: 2, reveur: 1 },
    ...
    if (nb == 10) {
        return {
            question: `Et si ${tu('tu pouvais', 'vous pouviez')} donner un conseil à ${tu(
                'ton',
                'votre'
            )} plus jeune ${tu('toi', 'vous')}, lequel serait-il? `,
            points: [
        ...
        return {
            question:
                ' Dernière question: ' +
                answers[0] +
                ` qu'${tu('as-tu', 'avez-vous')} pensé du quiz?`,
            points: [
                { reveur: 2 },
                { leader: 2 },
                { hermite: 2 },
...
function get_reponse(point_total: Personnalite): PersonnaliteStr {
    return Object.entries(point_total).reduce((acc, curr) =>
        acc[1] < curr[1] ? curr : acc
    )[0] as PersonnaliteStr;
}
...
function App() {
    ...
    return (
        <div className={styles.App}>
            <div className={'window'} style={{ maxHeight: '100%', minHeight: '100vh' }}>
                <div className="title-bar">
                    <div className="title-bar-text">Quizz : trouve ton identité plastique</div>
                ...
                <div className="window-body" style={{}}>
                    {question_number < 12 ? (
                            ...
                            callback={(a: string, nb: number, points: Personnalite[]) => {
                                if (points) {
                                    for (let p of Object.keys(points[nb])) {
                                        point_total[p as keyof typeof point_total]! +=
                                            points[nb][p as keyof typeof Personnalite];
                                    }
                        ...
                        <Personnalite personnalite={get_reponse(point_total)!} tu_vous={check_tu_vous(answers_nb) as ('tu' | "vous")}/>
                    )}
                </div>
            </div>
        </div>
